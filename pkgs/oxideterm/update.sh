...
cli_hash="$(probe_hash "(let pkg = (import ./default.nix { }).$package; src = pkg.cli.cargoDeps.vendorStaging; in (src.overrideAttrs (_: { outputHash = \"\"; outputHashAlgo = \"sha256\"; })))")"
...
sed -i -E '/cargoRoot = "cli"/,/cargoBuildFlags =/ s|cargoHash = [^;]+;|cargoHash = "'"$cli_hash"'";|' "$package_file"
sed -i -E '/cargoRoot = "src-tauri"/,/pnpmDeps = fetchPnpmDeps \{/ s|cargoHash = [^;]+;|cargoHash = "'"$main_hash"'";|' "$package_file"
